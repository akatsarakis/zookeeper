...
void *run_worker(void *arg)
{
	...
	uint8_t worker_sl = 0;
	int remote_client_num =  CLIENT_NUM - LEADERS_PER_MACHINE;
	...
	struct mica_resp mica_resp_arr[WORKER_MAX_BATCH], local_responses[LEADERS_PER_MACHINE * LOCAL_WINDOW], mica_batch_resp[WORKER_MAX_BATCH]; // this is needed because we batch to MICA all reqs between 2 writes
	struct ibv_send_wr wr[WORKER_MAX_BATCH], *bad_send_wr = NULL;
	struct ibv_sge sgl[WORKER_MAX_BATCH];

	struct ibv_wc wc[WS_PER_WORKER * (CLIENT_NUM - LEADERS_PER_MACHINE)], send_wc;
	...
	struct mica_op *local_op_ptr_arr[LEADERS_PER_MACHINE * LOCAL_WINDOW];
	for (i = 0; i < LEADERS_PER_MACHINE * LOCAL_WINDOW; i++)
		local_op_ptr_arr[i] = (struct mica_op*)(local_req_region + ((wrkr_lid * LEADERS_PER_MACHINE * LOCAL_WINDOW) + i));
