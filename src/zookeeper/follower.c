...
void *follower(void *arg)
{
    ...
    int clt_gid = (machine_id * LEADERS_PER_MACHINE) + params.id;	/* Global ID of this client thread */
    uint16_t local_client_id = (uint16_t) (clt_gid % LEADERS_PER_MACHINE);
...
//    printf("Remote clients buffer size %d\n", remote_buf_size);
    ...
    struct hrd_ctrl_blk *cb = hrd_ctrl_blk_init(clt_gid,	/* local_hid */
                                                ...
                                                MASTER_SHM_KEY + FOLLOWERS_PER_MACHINE + local_client_id, /* key */
...
//    printf("dgram pointer %llu, incoming req ptr %llu \n", cb->dgram_buf, incoming_reqs);
    ...
    uint16_t wn = 0, wr_i = 0, br_i = 0, cb_i = 0, coh_i = 0, coh_buf_i = 0, rem_req_i = 0, prev_rem_req_i,
            ...
            ws[FOLLOWERS_PER_MACHINE] = {0}; /* Window slot to use for LOCAL workers*/
