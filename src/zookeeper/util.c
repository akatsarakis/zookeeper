...
void createAHs(uint16_t clt_gid, struct hrd_ctrl_blk *cb)
{
    ...
    for (qp_i = 0; qp_i < WORKER_NUM_UD_QPS; qp_i++) {
        for(i = 0; i < WORKER_NUM; i++) {
            if (i / FOLLOWERS_PER_MACHINE == machine_id) continue; // skip the local workers
            ...
            sprintf(wrkr_name, "worker-dgram-%d-%d-%d", i / FOLLOWERS_PER_MACHINE, i % FOLLOWERS_PER_MACHINE, qp_i);
    ...
    for(i = 0; i < CLIENT_NUM; i++) {
        if (i / LEADERS_PER_MACHINE == machine_id) continue; // skip the local machine
...
void trace_init(struct trace_command **cmds, int clt_gid) {
    ...
    if (FEED_FROM_TRACE == 1) {
        ...
        if (LEADERS_PER_MACHINE <= 23) sprintf(local_client_id, "%d", (clt_gid % LEADERS_PER_MACHINE));
        ...
        parse_trace(path, cmds, clt_gid % LEADERS_PER_MACHINE);
...
int spawn_stats_thread() {
    ...
    CPU_ZERO(&cpus_stats);
    if(FOLLOWERS_PER_MACHINE + LEADERS_PER_MACHINE > 17)
        CPU_SET(39, &cpus_stats);
    else
        CPU_SET(2 *(FOLLOWERS_PER_MACHINE + LEADERS_PER_MACHINE) + 2, &cpus_stats);
...
{
    int i;
    int local_client_id = clt_gid % LEADERS_PER_MACHINE;
