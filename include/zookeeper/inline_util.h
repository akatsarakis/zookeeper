	...
	while (resp[(*op_i)].type != CACHE_MISS && resp[(*op_i)].type != UNSERVED_CACHE_MISS &&
		...
		if (resp[(*op_i)].type == CACHE_GET_SUCCESS || ops[(*op_i)].opcode == CACHE_OP_UPD) {// Empty the Reads and the broadcasts that have been sent
			if ((ENABLE_HOT_KEY_TRACKING == 1) && (resp[(*op_i)].type == CACHE_GET_SUCCESS)) {
				t_stats[local_client_id].cache_hits_per_thread += (ops[(*op_i)].val_len + 1);
			}
			else t_stats[local_client_id].cache_hits_per_thread++;
...
// Accidentally changed for zookeeper but no used yet
static inline void cache_hit_bookkeeping(struct extended_cache_op *ops, struct extended_cache_op *next_ops,
																				 struct mica_resp *resp,
																				 struct mica_resp *next_resp, struct key_home *key_homes,
																				 struct key_home *next_key_homes,
																				 ...
																				 struct latency_flags *latency_info, uint16_t *hottest_keys_pointers)
{
	while (resp[(*op_i)].type != CACHE_MISS && resp[(*op_i)].type != UNSERVED_CACHE_MISS && (*op_i) < CACHE_BATCH_SIZE) {
		...
		if (resp[(*op_i)].type == CACHE_GET_SUCCESS || ops[(*op_i)].opcode == CACHE_OP_INV ||
			...
			f_stats[t_id].cache_hits_per_thread++;
			resp[(*op_i)].type = EMPTY;
		}
		else {
			copy_op_to_next_op(ops, next_ops, resp, next_resp, key_homes, next_key_homes, (*op_i),
												 next_op_i, latency_info, hottest_keys_pointers);
		}
		(*op_i)++;
	}
}

// Bookkeeping of requests that hit in the cache for both leader and follower threads
static inline void run_through_rest_of_ops(struct extended_cache_op *ops, struct extended_cache_op *next_ops,
											   struct mica_resp *resp,
											   struct mica_resp *next_resp,
											   uint16_t *op_i, uint16_t *next_op_i,
											   uint16_t t_id,
											   struct latency_flags *latency_info, bool is_leader_t)
{
	while (*op_i < CACHE_BATCH_SIZE) {
		...
		else copy_op_to_next_op(ops, next_ops, resp, next_resp, NULL, NULL, (*op_i), next_op_i, latency_info, NULL);
		(*op_i)++;
	}
}
									...
									uint16_t wr_i)
{
	uint16_t i, j;
	int send_cntr = 0;
	uint16_t clt_gid = machine_id * LEADERS_PER_MACHINE + local_client_id;
	for (i = 0; i < wr_i; i++) {
															 ...
															 int protocol)
{
	...
	if (protocol == LEADER) max_credit_recvs = MAX_CREDIT_RECVS;
	else if (protocol == FOLLOWER) max_credit_recvs = SC_MAX_CREDIT_RECVS;
							   ...
							   int local_client_id, uint8_t per_worker_outstanding, struct ibv_send_wr* rem_send_wr)
{
	...
	if (ENABLE_COALESCING == 1 && ops[op_i].opcode == CACHE_OP_GET) {
		if (coalesce_struct[rm_id].slots == 0 && per_worker_outstanding < WS_PER_WORKER) {
			...
			if ((LEADER_ENABLE_INLINING == 0) && rem_send_sgl[cl_wr_i].length > MAXIMUM_INLINE_SIZE) {
			...
			if (ENABLE_ASSERTIONS == 1) {
				...
				assert(tmp->clt_gid == local_client_id + LEADERS_PER_MACHINE * machine_id);
								...
								struct local_latency* local_measure)
{
	if (ENABLE_LOCAL_WORKERS) {
		wn = local_worker_id;
		int rm_id = worker_id / FOLLOWERS_PER_MACHINE;
		worker_id = rm_id * FOLLOWERS_PER_MACHINE + wn;
	}
	else if (ENABLE_WORKERS_CRCW && ENABLE_STATIC_LOCAL_ALLOCATION) {
		wn = local_client_id % FOLLOWERS_PER_MACHINE;
		int rm_id = worker_id / FOLLOWERS_PER_MACHINE;
		worker_id = rm_id * FOLLOWERS_PER_MACHINE + wn;
	}
	...
	if (local_recv_flag[wn][local_client_id][loc_region] == 0) {
		...
		t_stats[local_client_id].locals_per_client++;
		MOD_ADD(ws[wn], LOCAL_WINDOW);
		resp[op_i].type = EMPTY;
	}
	else {
								...
								void* buf)
{
	...
	for(i = 0; i < credit_wr_i; i++) {
		for (j = 0; j < credits_in_message; j++) {
			...
			MOD_ADD(*push_ptr, clt_buf_slots);
	...
	do {
		...
		if(responses_found != 0) {
			...
			for (j = 0; j < responses_found; j++) {
				if (ENABLE_ASSERTIONS == 1) assert(wc[j].imm_data < FOLLOWER_NUM);
		...
		t_stats[local_client_id].stalled_time_per_client++;
	...
	if ((outstanding_rem_reqs + wr_i) >= MAX_OUTSTANDING_REQS) { //   the client has no credits at all
		...
		if (BALANCE_REQS == 1) { // option 1: send to an alternative worker TODO test this!
			...
			for (i = 0; i < FOLLOWERS_PER_MACHINE; i++) {
				if (per_worker_outstanding[rm_id * FOLLOWERS_PER_MACHINE + i] < WS_PER_WORKER) {
...
{
	rem_send_wr[wr_i].wr.ud.ah = remote_leader_qp[worker_qp_i][worker_id].ah;
	rem_send_wr[wr_i].wr.ud.remote_qpn = remote_leader_qp[worker_qp_i][worker_id].qpn;
											...
											uint16_t* hottest_keys_pointers)
{
	...
	while ((*rem_req_i) < WINDOW_SIZE && op_i < CACHE_BATCH_SIZE) {
		if (DISABLE_CACHE != 1) {
			if (protocol == LEADER)
				cache_hit_bookkeeping(ops, next_ops, resp, next_resp, key_homes, next_key_homes,
...
//									hottest_keys_pointers);
//	else if (protocol == FOLLOWER)
//		run_through_rest_of_ops(ops, next_ops, resp, next_resp, key_homes, next_key_homes,
//								   &op_i, next_op_i, local_client_id, latency_info,
//								   hottest_keys_pointers);
										 ...
										 struct ibv_sge* rem_send_sgl, struct latency_flags* latency_info, struct timespec* start)
{
	...
	if (wr_i > 0) {
		if (ENABLE_ASSERTIONS) {
			...
			assert(wr_i <= FOLLOWERS_PER_MACHINE * WS_PER_WORKER * (MACHINE_NUM- 1));
		...
		t_stats[local_client_id].remote_messages_per_client += wr_i;
		//		if (ENABLE_WORKER_COALESCING == 0)
		//			t_stats[local_client_id].remotes_per_client += wr_i;
		*outstanding_rem_reqs += wr_i;
		...
		t_stats[local_client_id].batches_per_client++;

	}
	else if (ENABLE_STAT_COUNTING == 1) t_stats[local_client_id].wasted_loops++;
}
									 ...
									 int local_client_id)
{
	while (*coh_i < BCAST_TO_CACHE_BATCH) {
		// printf("CLIENT %d Opcode %d at offset %d at address %u \n", clt_gid, incoming_reqs[(*slots_ahead + 1) % SC_CLT_BUF_SLOTS].m_op.opcode,
		// 			 (*slots_ahead + 1) % SC_CLT_BUF_SLOTS, &(incoming_reqs[(*slots_ahead + 1) % SC_CLT_BUF_SLOTS]));
		...
		MOD_ADD(*pull_ptr, SC_CLT_BUF_SLOTS);
		...
		// 		clt_gid, t_stats[local_client_id].received_updates_per_client,* slots_ahead, update_ops[*coh_i].key.tag);
		incoming_reqs[*pull_ptr].m_op.opcode = 0;
		(*coh_i)++;
		t_stats[local_client_id].received_updates_per_client++;
	}
}
										 ...
										 struct ibv_cq *credit_send_cq)
{
	...
	for (i = 0; i < coh_i; i++) {
		...
		if (broadcasts_seen[rm_id] == SC_CREDITS_IN_MESSAGE) {
			uint16_t clt_i = rm_id * LEADERS_PER_MACHINE + local_client_id;
			credit_send_wr[credit_wr_i].wr.ud.ah = remote_leader_qp[clt_i][FC_UD_QP_ID].ah;
			credit_send_wr[credit_wr_i].wr.ud.remote_qpn = remote_leader_qp[clt_i][FC_UD_QP_ID].qpn;
...
{
	struct ibv_wc signal_send_wc;
	t_stats[local_client_id].updates_per_client++;
	...
	if (LEADER_ENABLE_INLINING == 1)	coh_send_sgl[br_i].addr = (uint64_t) (uintptr_t) (ops + op_i);
	if (LEADER_ENABLE_INLINING == 0) {
									  ...
									  struct small_cache_op *inv_ops)
{
	while (*polled_messages < BCAST_TO_CACHE_BATCH) {
		// printf("CLIENT %d Opcode %d at offset %d at address %u \n", clt_gid, incoming_reqs[(*slots_ahead + 1) % LIN_CLT_BUF_SLOTS].m_op.opcode,
		// 			(*slots_ahead + 1) % LIN_CLT_BUF_SLOTS, &(incoming_reqs[(*slots_ahead + 1) % LIN_CLT_BUF_SLOTS]));
		...
		if (incoming_reqs[(*pull_ptr + 1) % LIN_CLT_BUF_SLOTS].m_op.opcode != CACHE_OP_INV) {
			...
			MOD_ADD(*pull_ptr, LIN_CLT_BUF_SLOTS);
			...
			//				 yellow_printf("CLIENT %d I see a message %s for key %d with version %d\n", local_client_id, code_to_str(incoming_reqs[*slots_ahead].m_op.opcode),
			...
			if (incoming_reqs[*pull_ptr].m_op.opcode == CACHE_OP_ACK) {
				...
				ack_ops_i++;
				if (ENABLE_STAT_COUNTING == 1) t_stats[local_client_id].received_acks_per_client++;
			}
			...
			(*inv_ops_i)++;
			if (ENABLE_STAT_COUNTING == 1) t_stats[local_client_id].received_invs_per_client++;
		}
												...
												uint16_t* ack_push_ptr, uint16_t* ack_size)
{
	...
	for (i = 0; i < update_ops_i; i++) {
		...
		if (update_ops[i].opcode == CACHE_OP_UPD) {
			...
			MOD_ADD(*ack_push_ptr, BCAST_TO_CACHE_BATCH);
			(*ack_size)++;
									   ...
									   uint16_t *updates_sent, uint16_t br_i, uint8_t credits[][MACHINE_NUM], uint8_t vc)
{
	...
	if (op_i < CACHE_BATCH_SIZE) {
		...
		t_stats[local_client_id].invs_per_client++;
		if (LEADER_ENABLE_INLINING == 0) {
		...
		if (LEADER_ENABLE_INLINING == 1)	coh_send_sgl[br_i].addr = (uint64_t) (uintptr_t) (ack_bcast_ops + (*ack_pop_ptr));
		if (LEADER_ENABLE_INLINING == 0) {
			if (ENABLE_ASSERTIONS == 1) assert(*coh_buf_i < COH_BUF_SLOTS);
		...
		MOD_ADD(*ack_pop_ptr, BCAST_TO_CACHE_BATCH);
	}
	// Do a Signaled Send every BROADCAST_SS_BATCH broadcasts (BROADCAST_SS_BATCH * (MACHINE_NUM - 1) messages)
							 ...
							 uint16_t coh_message_count[][MACHINE_NUM], struct hrd_ctrl_blk* cb, uint16_t debug_ptr)
{
	...
	for (i = 0; i < BCAST_TO_CACHE_BATCH; i++) {
		...
		remote_clt_id = inv_to_send_ops[i].key.meta.cid * LEADERS_PER_MACHINE + local_client_id;
		...
		ack_wr[send_ack_count].wr.ud.ah = remote_leader_qp[remote_clt_id][BROADCAST_UD_QP_ID].ah;
		ack_wr[send_ack_count].wr.ud.remote_qpn = remote_leader_qp[remote_clt_id][BROADCAST_UD_QP_ID].qpn;
		...
		t_stats[local_client_id].acks_per_client++;
		...
		if (fc_message_count > 0) {
			...
			for (i = 0; i < fc_message_count; i++) {
				...
				if (acks_seen[rm_id] == CREDITS_IN_MESSAGE || invs_seen[rm_id] == CREDITS_IN_MESSAGE || upds_seen[rm_id] == CREDITS_IN_MESSAGE) {
					uint16_t clt_i = rm_id * LEADERS_PER_MACHINE + local_client_id;
					credit_wr[credit_wr_i].wr.ud.ah = remote_leader_qp[clt_i][FC_UD_QP_ID].ah;
					credit_wr[credit_wr_i].wr.ud.remote_qpn = remote_leader_qp[clt_i][FC_UD_QP_ID].qpn;
...
{
//  printf("Ldr %d pulling the trace \n", t_id);
	int i = 0, op_i = 0;
	uint8_t is_update = 0;
  ...
  printf("working session = %d\n", working_session);
    ...
    //ops[op_i].key.meta.cid = working_session;
		ops[op_i].opcode = is_update ? (uint8_t) CACHE_OP_PUT : (uint8_t) CACHE_OP_GET;
		ops[op_i].val_len = is_update ? (uint8_t) (VALUE_SIZE >> SHIFT_BITS) : (uint8_t) 0; // if it is not an update the val_len will not be included in the packet
		if (is_update) {
...
//		if (ENABLE_ASSERTIONS) assert(p_writes->g_id[i] == 0); //TODO is this needed?
...
static inline void poll_for_acks(struct ack_message_ud_req *incoming_acks, uint32_t *pull_ptr,
																 ...
																 struct recv_info *ack_recv_info, uint16_t t_id)
{
//  if (p_writes->size > 0) printf("Ldr %d p_writes->size %u, polling for acks \n", t_id, p_writes->size);
	...
	while (incoming_acks[index].ack.opcode == CACHE_OP_ACK) {
		...
		credits[COMM_VC][ack->follower_id] += ack_num;
    if (credits[PREP_VC][ack->follower_id] > PREPARE_CREDITS) credits[PREP_VC][ack->follower_id] = PREPARE_CREDITS;
...
//		while ((*(uint64_t *) (ack->local_id)) == 0); // TODO this is problematic because lids start from 0
...
//    yellow_printf("Leader %d ack opcode %d with %d acks for l_id %lu, oldest lid %lu, at offset %d at address %u \n",
//                  t_id, ack->opcode, ack_num, l_id, pull_lid, index, &(incoming_acks[index]));
...
//               t_id, ack_i, ack_num,  ack_ptr, p_writes->g_id[ack_ptr]);
  ...
  if (polled_messages > 0) hrd_poll_cq(ack_recv_cq, polled_messages, ack_recv_wc);
...
static inline void propagate_updates(struct pending_writes *p_writes,
																		 struct commit_fifo *com_fifo, struct mica_resp *resp, uint16_t t_id)
{
...
//    printf("Leader found updates to propagate\n");
    ...
    uint32_t prep_fifo_pull_ptr = p_writes->prep_fifo->pull_ptr;
    if (p_writes->prep_fifo->backward_ptrs[prep_fifo_pull_ptr] == p_writes->pull_ptr) {
      p_writes->prep_fifo->size--;
      MOD_ADD(p_writes->prep_fifo->pull_ptr, PREP_FIFO_SIZE);
    }
																	 ...
																	 struct ibv_cq * w_recv_cq, struct ibv_wc *w_recv_wc)
{
	...
	while (incoming_ws[index].writes[0].opcode == CACHE_OP_PUT) {
		...
		if(w_num > 1) {
			while (writes[w_num - 1].opcode != CACHE_OP_PUT); // spin until the entire message is there
		}
		for (uint16_t i = 0; i < w_num; i++) {
			uint16_t push_ptr = p_writes->push_ptr;
			p_writes->w_state[push_ptr] = VALID;
			memcpy(&p_writes->session_id, (uint32_t *) &(writes[i].session_id), sizeof(uint32_t));
			MOD_ADD(p_writes->push_ptr, LEADER_PENDING_WRITES);
		}
...
static inline void post_recvs(struct ibv_sge* recv_sgl,
															...
															uint32_t slot_size,
															void* buf, uint16_t recv_num)
{
	uint16_t j;
	struct ibv_recv_wr *bad_recv_wr;
	for (j = 0; j < recv_num; j++) {
		recv_sgl[j].addr = (uintptr_t) buf + ((*push_ptr) * slot_size);
		MOD_ADD(*push_ptr, buf_slots);
		//			if (*push_ptr == 0) *push_ptr = 1;
		recv_wr[j].next = (j == recv_num - 1) ?
											NULL : &recv_wr[j + 1];
	}
	int ret = ibv_post_recv(recv_qp, &recv_wr[0], &bad_recv_wr);
	CPE(ret, "ibv_post_recv error", ret);
}
                                       ...
                                       uint32_t* credit_debug_cnt, uint8_t vc, int protocol)
{
  ...
  // There are no explicit credit messages
                                   ...
                                   uint8_t vc)
{
  ...
  if (op_i < CACHE_BATCH_SIZE) { // Prepare message
    ...
    coh_send_sgl[br_i].length = HERD_PUT_REQ_SIZE;
    t_stats[t_id].invs_per_client++;

    if (LEADER_ENABLE_INLINING == 1)	coh_send_sgl[br_i].addr = (uint64_t) (uintptr_t) (w_ops + op_i);
    if (LEADER_ENABLE_INLINING == 0) {
      if (ENABLE_ASSERTIONS == 1) assert(*coh_buf_i < COH_BUF_SLOTS);
      memcpy(coh_buf + (*coh_buf_i), w_ops + op_i, HERD_PUT_REQ_SIZE);
      coh_send_sgl[br_i].addr = (uint64_t) (uintptr_t) (coh_buf + (*coh_buf_i));
      MOD_ADD_WITH_BASE(*coh_buf_i, COH_BUF_SLOTS, 0);
      for (i = 0; i < MESSAGES_IN_BCAST; i++)
        coh_send_wr[(br_i * MESSAGES_IN_BCAST) + i].send_flags = 0;
    }
    		 green_printf("THREAD %d : I BROADCAST a %s with  credits: %d, gid: %d  \n", t_id,
                      code_to_str(w_ops[op_i].opcode), credits[vc][(machine_id + 1) % MACHINE_NUM],
                      w_ops[op_i].key.tag, w_ops[op_i].g_id);
  }


  // --------------COMMIT----------------
  else { // The commit messages are small and thus always inlined (NIC always reads synchronously)
    if (LEADER_ENABLE_INLINING == 0) {
      for (i = 0; i < MESSAGES_IN_BCAST; i++)
        coh_send_wr[(br_i * MESSAGES_IN_BCAST) + i].send_flags = IBV_SEND_INLINE;// we should inline commit
    }
    coh_send_sgl[br_i].addr = (uint64_t) (uintptr_t) (ack_bcast_ops + (*ack_pop_ptr));
    coh_send_sgl[br_i].length = HERD_PUT_REQ_SIZE;
    (*updates_sent)++;
    t_stats[t_id].updates_per_client++;
    //      green_printf("CLIENT %d : I BROADCAST with %s credits: %d, key: %d, cid: %d and version: %d \n", t_id, code_to_str(ack_bcast_ops[*ack_pop_ptr].opcode), credits[vc][(machine_id + 1) % MACHINE_NUM],
    //                   ack_bcast_ops[*ack_pop_ptr].key.tag, ack_bcast_ops[*ack_pop_ptr].key.meta.cid, 	ack_bcast_ops[*ack_pop_ptr].key.meta.version);
    MOD_ADD(*ack_pop_ptr, BCAST_TO_CACHE_BATCH);
    (*commit_br_tx)++;
  }
...
static inline void forge_prep_wr(uint16_t prep_i, struct pending_writes *p_writes,
	...
	green_printf("Leader %d : I BROADCAST a prepare message %d of %u prepares with  credits: %d, lid: %u  \n",
							 t_id, prep->opcode, coalesce_num, credits[vc][0],  *(uint32_t*)prep->l_id);
...
static inline void broadcast_prepares(struct pending_writes *p_writes,
																			...
																			long long *prep_br_tx, struct recv_info *ack_recv_info, uint16_t t_id)
{
...
//    printf("LDR %d has %u bcasts to send credits %d\n",t_id, p_writes->prep_fifo->bcast_size, credits[PREP_VC][0]);
    ...
    if (ENABLE_ASSERTIONS) assert( p_writes->prep_fifo->bcast_size >= coalesce_num);
...
//  if (t_id == 0) printf("index %u \n", index);
	while(incoming_preps[index].prepare.opcode == CACHE_OP_PUT && p_writes->size < FLR_PENDING_WRITES) {
		...
		if (p_writes->size + coalesce_num > FLR_PENDING_WRITES) return;
...
//           t_id, coalesce_num, index, incoming_l_id, expected_l_id);
		if (FLR_DISALLOW_OUT_OF_ORDER_PREPARES) assert(expected_l_id == (uint64_t) incoming_l_id);
		...
		if (expected_l_id < (uint64_t) incoming_l_id) {
			...
			if (p_writes->size + extra_slots + coalesce_num > FLR_PENDING_WRITES) return;
		}
		...
		while (prepare[coalesce_num - 1].opcode == 0);
		for (uint8_t prep_i = 0; prep_i < coalesce_num; prep_i++) {
			...
			green_printf("Flr %u, new write at ptr %u with g_id %lu and flr id %u  \n",
									 t_id, push_ptr, p_writes->g_id[push_ptr], p_writes->flr_id[push_ptr]);
			if (ENABLE_ASSERTIONS) {
...
//                t_id, l_id_to_send, ack->ack_num, ack->follower_id, p_writes->size, FLR_PENDING_WRITES);
  if (ENABLE_ASSERTIONS) assert(ack->ack_num > 0);
	...
	posted_recvs = com_recv_info->posted_recvs;
	recvs_to_post_num = MIN((FLR_MAX_RECV_COM_WRS - posted_recvs), send_ack_count);
	if (recvs_to_post_num > 0) {
...
static inline void poll_for_coms(struct com_message_ud_req *incoming_coms, uint32_t *pull_ptr,
                                 ...
                                 struct recv_info *com_recv_info, uint16_t t_id, uint8_t flr_id)
{
  uint32_t index = *pull_ptr;
  uint32_t polled_messages = 0;
  while (incoming_coms[index].com.opcode == CACHE_OP_PUT) {
    ...
    // spin until the entire message is there
//		while ((*(uint64_t *) (com->local_id)) == 0); // TODO this is problematic because lids start from 0
    ...
    if (ENABLE_ASSERTIONS) {
			assert(l_id + com_num <= pull_lid + p_writes->size);
    ...
    for (uint16_t com_i = 0; com_i < com_num; com_i++) {
      if (ENABLE_ASSERTIONS) assert(p_writes->w_state[com_ptr] == VALID);
      printf("Flr %d valid com %u/%u write at ptr %d with g_id %lu is ready \n",
			...
			if (p_writes->flr_id[com_ptr] == flr_id) {assert(false);(*credits)++;}
      MOD_ADD(com_ptr, FLR_PENDING_WRITES);
    }
    if (ENABLE_ASSERTIONS) assert(*credits <= W_CREDITS);
    memset(com, 0, LDR_COM_SEND_SIZE); // need to delete all the g_ids
...
//			f_stats[wrkr_lid].locals_per_worker += LOCAL_REGION_SIZE;
//		}
//	}
//}
//
//static inline void worker_remote_request_error_checking(int slots_ahead, uint32_t per_qp_buf_slots, struct mica_op* next_req_ptr,
...
//{
//	if (ENABLE_ASSERTIONS == 1) {
//		assert(slots_ahead <= (int)per_qp_buf_slots);
...
//				yellow_printf("Wrker %d: qp_i %d, slots_ahead %d, wr_i %d, len %d, op %d, total reqs %llu\n",
//							  wrkr_lid, qp_i,	slots_ahead, wr_i, req->m_op.val_len,
//							  next_req_ptr->opcode, nb_tx_tot );
//				assert(false);
//			}
//		}
//	}
//	//printf("gets %d\n", req[slots_ahead].m_op.value[0]);
//
...
//			assert(false);
//		}
//		assert(clt_i / LEADERS_PER_MACHINE != machine_id);
//	}
//}
//
//// poll the remote request region and return a control flow directive
//static inline enum control_flow_directive poll_remote_region(uint16_t* qp_i, int* slots_ahead, uint32_t* per_qp_buf_slots, struct wrkr_ud_req* req,
...
//{
//	uint32_t index = (slots_ahead[(*qp_i)] + 1) % per_qp_buf_slots[(*qp_i)];
...
//					assert(false);
//				}
//			if ((*qp_i)  == FOLLOWER_QP_NUM -1) return break_; // all qp buffers were checked
//			else {
//				MOD_ADD((*qp_i) , FOLLOWER_QP_NUM);
...
//static inline void request_bookkeeping(bool multiget, int* slots_ahead, uint16_t qp_i, uint32_t* per_qp_buf_slots,
...
//	if (!multiget) {
//		MOD_ADD_WITH_BASE(slots_ahead[qp_i], per_qp_buf_slots[qp_i], 0);
//		(*index) = slots_ahead[qp_i];
//		(*next_req_ptr) = (struct mica_op *) &(req[*index].m_op);
...
//		wr[wr_i].wr.ud.ah = remote_leader_qp[(*clt_i)][REMOTE_UD_QP_ID].ah;
//		wr[wr_i].wr.ud.remote_qpn = remote_leader_qp[(*clt_i)][REMOTE_UD_QP_ID].qpn;
//
//		//		green_printf("Creating a request %d for client %d\n", wr_i, *clt_i);
//
//		if (ENABLE_ASSERTIONS)
//			worker_remote_request_error_checking(slots_ahead[qp_i], per_qp_buf_slots[qp_i], *next_req_ptr,
...
//																   int* slots_ahead, uint16_t* qp_i, uint32_t per_qp_buf_slots, uint16_t per_qp_received_messages,
...
//		(*multiget) = false;
//		MOD_ADD_WITH_BASE(slots_ahead[(*qp_i)], per_qp_buf_slots, 0);
//		// if (wrkr_lid == 0)
//		// 	hrd_green_printf("Multiget from clt %d is over wr_i %d, received messages %d, slots_ahead %d, get_num %d\n",
//		// 			clt_i, wr_i, received_messages, slots_ahead[(*qp_i)],get_num );
//	}
//	if (FOLLOWER_QP_NUM > 1) {
//		if (per_qp_received_messages== per_qp_buf_slots && (*multiget) == false) {
//			if ((*qp_i) == FOLLOWER_QP_NUM -1 ) return break_; // all qp buffers were checked
//			else MOD_ADD_WITH_BASE((*qp_i), FOLLOWER_QP_NUM, 0);
//		}
//	}
...
//	for (qp_i = 0; qp_i < FOLLOWER_QP_NUM; qp_i++) { // take care to poll the correct recv queues
...
//			if (ENABLE_ASSERTIONS) {
//				assert(per_qp_received_messages[qp_i] <= WS_PER_WORKER * (CLIENT_NUM - LEADERS_PER_MACHINE));
//				if (FOLLOWER_QP_NUM == 1) assert(per_qp_received_messages[qp_i] == received_messages);
//			}
...
//		if(FOLLOWER_QP_NUM == 1 && nb_new_req_tot > 0 && ENABLE_ASSERTIONS)
...
//		f_stats[wrkr_lid].batches_per_worker++;
//		f_stats[wrkr_lid].remotes_per_worker+= total_reqs_sent;
//		if (MEASURE_LATENCY == 1) {
//			wr[last_measured_wr_i].opcode = IBV_WR_SEND;
//		}
//		MOD_ADD_WITH_BASE(send_qp_i, FOLLOWER_QP_NUM, 0);
//	}
//}
