									 ...
									 int local_client_id)
{
	while (*coh_i < BCAST_TO_CACHE_BATCH) {
		...
		// 		clt_gid, t_stats[local_client_id].received_updates_per_client,* pull_ptr, update_ops[*coh_i].key.tag);
                                                        ...
                                                        struct latency_flags *latency_info, struct timespec *start)
{
  ...
  if (p_writes->writes_num == LEADER_PENDING_WRITES) return trace_iter;
	for (i = 0; i < SESSIONS_PER_THREAD; i++) {
		if (p_writes->w_state[i] == INVALID ) {
			working_session = i;
			break;
		}
	}
...
//      printf("Thread %d found a write for session %d\n", t_id, working_session);
      p_writes->writes_num++;
      p_writes->unordered_writes[p_writes->unordered_writes_num] = working_session;
      p_writes->unordered_writes_num++;
      struct write_op *tmp_op = (struct write_op*) &ops[op_i];
      memcpy(&(p_writes->write_ops[working_session].key), &(tmp_op->key), sizeof(struct key));
      if (ENABLE_ASSERTIONS) {
        assert(p_writes->w_state[working_session] == INVALID);
        assert(keys_are_equal((struct cache_key *)&p_writes->write_ops[working_session], &ops[op_i].key) == 1);
      }
      p_writes->w_state[working_session] = VALID;
      while (p_writes->w_state[working_session] != INVALID) {
        working_session++;
...
static inline void get_wids_depricated(struct pending_writes *p_writes, struct completed_writes *c_writes, uint16_t t_id)
{
  ...
  for (i = 0; i < p_writes->unordered_writes_num; ++i) {
    ...
    c_writes->p_writes_ptr[c_writes->push_ptr] = write_op_ptr;
...
// Poll for acks in the leader
static inline void poll_for_acks_depricated(struct ack_message_ud_req *incoming_acks, int *pull_ptr,
                                 ...
                                 struct ibv_cq * ack_recv_cq, struct ibv_wc *ack_recv_wc)
{
  ...
  while (incoming_acks[index].ack.opcode == CACHE_OP_ACK) {
    ...
    if (LDR_ACK_RECV_SIZE > CACHE_LINE_SIZE) {
      while ((*(uint64_t *)(ack->global_id + (ack_num - 1) * sizeof(uint64_t))) == 0); // spin until the entire message is there
    }
    uint16_t  ack_i, w_i;
    for (ack_i = 0; ack_i < ack_num; ack_i++) {
      uint64_t g_id = *(uint64_t *)(ack->global_id + ack_i * sizeof(uint64_t));
...
static inline void forge_commit_messsage_depricated(struct commit_fifo *com_fifo, struct completed_writes *c_writes)
{
	uint16_t com_mes_i = com_fifo->total_push_ptr;
	struct com_message *commits = com_fifo->commits;
	uint16_t g_id_i = commits[com_mes_i].com_num;
	uint16_t w_ops_ptr = c_writes->pull_ptr;
	commits[com_mes_i].g_id[g_id_i] = c_writes->w_ops[w_ops_ptr]->g_id;
	commits[com_mes_i].com_num++;
	com_fifo->size++;
	if (commits[com_mes_i].com_num == MAX_COM_COALESCE)
		MOD_ADD(com_fifo->total_push_ptr, COMMIT_FIFO_SIZE);
	if (ENABLE_ASSERTIONS) {
		assert(commits[com_mes_i].com_num <= MAX_COM_COALESCE);
		assert(com_fifo->size <= MAX_COM_COALESCE * COMMIT_FIFO_SIZE);
	}
}

// Propagate Updates that have seen all acks to the KVS
static inline void propagate_updates_depricated(struct completed_writes *c_writes, struct pending_writes *p_writes,
																		 struct commit_fifo *com_fifo, struct mica_resp *resp)
{
	uint16_t update_op_i = 0;
	uint32_t pull_ptr = c_writes->pull_ptr;
	//struct com_message *commits = com_fifo->commits;
	uint64_t committed_g_id = atomic_load_explicit(&committed_global_w_id, memory_order_relaxed);
	while(c_writes->w_state[c_writes->pull_ptr] == READY) {
		if (com_fifo->size == MAX_COM_COALESCE * COMMIT_FIFO_SIZE) break; // the commit fifo is full
		if (c_writes->w_ops[c_writes->pull_ptr]->g_id != committed_g_id + 1) break;

		c_writes->w_state[c_writes->pull_ptr] = INVALID;
		p_writes->w_state[c_writes->p_writes_ptr[c_writes->pull_ptr]] = INVALID;
		// Add the acked gid to the appropriate commit message
		forge_commit_messsage_depricated(com_fifo, c_writes);
		MOD_ADD(c_writes->pull_ptr, LEADER_PENDING_WRITES);
		update_op_i++;
		committed_g_id++;
	}
	if (update_op_i > 0) {
		cache_batch_op_updates((uint32_t) update_op_i, 0, (struct cache_op **) &(c_writes->w_ops[pull_ptr]), resp);
																						...
																						struct ibv_cq * ack_recv_cq, struct ibv_wc *ack_recv_wc)
{
	...
	while (incoming_acks[index].ack.opcode == CACHE_OP_ACK) {
		...
		for (uint16_t ack_i = 0; ack_i < ack_num; ack_i++) {
			...
			uint32_t last_w_ptr = (p_writes->push_ptr - 1) % LEADER_PENDING_WRITES;
			if (g_id < p_writes->write_ops[w_pull_ptr].g_id) continue;
			...
			while (p_write_id <= last_w_ptr) {
				if (g_id == p_writes->write_ops[p_write_id].g_id) {
					...
					p_writes->acks_seen[p_write_id]++;
					if (p_writes->acks_seen[p_write_id] == LDR_QUORUM_OF_ACKS)
						p_writes->w_state[p_write_id] = READY;
					break;
				}
				else p_write_id++;
			}
...
// Add the acked gid to the appropriate commit message
static inline void forge_commit_message(struct commit_fifo *com_fifo, uint64_t g_id)
{
	uint16_t com_mes_i = com_fifo->total_push_ptr;
	struct com_message *commits = com_fifo->commits;
	uint16_t g_id_i = commits[com_mes_i].com_num;

	commits[com_mes_i].g_id[g_id_i] = g_id;
	commits[com_mes_i].com_num++;
	com_fifo->size++;
	if (commits[com_mes_i].com_num == MAX_COM_COALESCE)
		MOD_ADD(com_fifo->total_push_ptr, COMMIT_FIFO_SIZE);
																		 ...
																		 struct commit_fifo *com_fifo, struct mica_resp *resp)
{
	...
	while(p_writes->w_state[p_writes->pull_ptr] == READY) {
		...
		// Add the acked gid to the appropriate commit message
		forge_commit_message(com_fifo, p_writes->write_ops[p_writes->pull_ptr].g_id);
																	 ...
																	 struct ibv_cq * w_recv_cq, struct ibv_wc *w_recv_wc)
{
	...
	while (incoming_ws[index].writes[0].opcode == CACHE_OP_PUT) {
		...
		if(w_num > 1) {
			while (writes[w_num].opcode != CACHE_OP_PUT); // spin until the entire message is there
		}
...
static inline void forge_commit_wrs(struct com_message *commit,
                                   uint16_t com_i, struct hrd_ctrl_blk *cb, struct ibv_sge *com_send_sgl,
                                   struct ibv_send_wr *com_send_wr,  long long *commit_br_tx,
                                    uint16_t credits[][FOLLOWER_MACHINE_NUM])
{
  struct ibv_wc signal_send_wc;
  com_send_sgl[com_i].addr = (uint64_t) (uintptr_t) commit;
  com_send_sgl[com_i].length = sizeof(uint64_t) * (commit->com_num + 1);
  //green_printf("Leader %d : I BROADCAST a message with %d commits with %s credits: %d \n",
     //          commit->com_num, code_to_str(commits[com_i].opcode), credits[COMM_VC][0]);
                                     ...
                                     struct ibv_recv_wr *credit_recv_wr)
{
  ...
  while (com_fifo->size > 0) {
    ...
    if (!check_bcast_credits(credits, cb, credit_wc, credit_debug_cnt, vc, protocol))
      break;
		struct com_message *commit = &com_fifo->commits[com_fifo->pull_ptr];
    // Create the broadcast messages
    forge_commit_wrs(commit, com_mes_i, cb, com_send_sgl,
    ...
    com_mes_i++;
    commits_sent += commit->com_num;
		com_fifo->size -= commit->com_num;
		MOD_ADD(com_fifo->pull_ptr, COMMIT_FIFO_SIZE);
    br_i++;
		if (ENABLE_ASSERTIONS) {
			assert(com_mes_i <= COMMIT_CREDITS);
			assert(com_fifo->size <= COMMIT_FIFO_SIZE * MAX_COM_COALESCE);
			assert(commit->com_num > 0 && commit->com_num < MAX_COM_COALESCE);
		}
		commit->com_num = 0;
    if (br_i == MAX_BCAST_BATCH) {
                              ...
                              void* buf, uint16_t recv_num)
{
  ...
  for (j = 0; j < recv_num; j++) {
    recv_sgl[j].addr = (uintptr_t) buf + ((*push_ptr) * UD_REQ_SIZE);
...
static inline void perform_broadcasts(uint16_t *ack_size, struct pending_writes *p_writes,
                                      ...
                                      uint16_t *coh_buf_i, long long *br_tx, long long *commit_br_tx,
                                      struct ibv_recv_wr *credit_recv_wr, uint16_t t_id, int protocol,
                                      struct ibv_sge* coh_recv_sgl, int* push_ptr, struct ibv_recv_wr* coh_recv_wr,
                                      struct ibv_qp* coh_recv_qp, uint32_t clt_buf_slots, void* buf)
{
  uint8_t vc;
  uint16_t updates_sent = 0, op_i = 0, br_i = 0, j, credit_recv_counter = 0;
  int prepare_num = -1;
//  struct cache_op *w_ops = p_writes->w_ops;
  while (op_i < LEADER_PENDING_WRITES + (*ack_size)) {
    ...
    if (op_i < LEADER_PENDING_WRITES) { // prepare messages
      if (p_writes->w_state[op_i] != VALID) {
        op_i++;
        continue;
      }
      vc = INV_VC;
    ...
    // Check if there are enough credits for a Broadcast
    if (!check_bcast_credits(credits, cb, credit_wc, credit_debug_cnt, vc, protocol)) {
      if (op_i >= LEADER_PENDING_WRITES) break;
      else {
        op_i = LEADER_PENDING_WRITES;
        continue;
      } // if there are no inv credits go on to the upds
    }
    // Create the broadcast messages
    forge_bcast_wrs(op_i, p_writes, ack_bcast_ops, ack_pop_ptr, cb, coh_send_sgl, coh_send_wr,
                    coh_message_count, coh_buf, coh_buf_i, br_tx, commit_br_tx, t_id, &updates_sent, br_i,
                    credits, vc);
    for (j = 0; j < MACHINE_NUM; j++) { credits[vc][j]--; }
    if ((*commit_br_tx) % CREDITS_IN_MESSAGE == 0) credit_recv_counter++;
    br_i++;
    if (op_i < LEADER_PENDING_WRITES) prepare_num++;
    op_i++;
    if (br_i == MAX_BCAST_BATCH) {
      if (prepare_num > 0) {
